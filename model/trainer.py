...
from model.ProRel import ProRel
from model.SingleDomainBaseline import DGCF, DICE, LightGCN


class Trainer():
    def __init__(self, opt, source_adj, target_adj, cross_payload, source_UV, source_VU, target_UV, target_VU):
        if opt['model_name'] == 'ProRel':
            self.model = ProRel(opt, source_adj, target_adj, cross_payload).cuda()
